...
void ui_frame(struct AppState *S) {
  ...
  if (nk_begin(
    ...
    if (!S->drv_win_open && !S->fp_win_open && !S->usermode_warn_open
        && S->theme_panel_anim < 0.01f)
      nk_window_set_focus(ctx, "Root");

    ui_titlebar(ctx, S);

    nk_layout_row_dynamic(ctx, 1, 1);
    ui_topbar(ctx, &S->active);

    nk_layout_row_dynamic(ctx, (float)wh - 96, 1); // height: высота области страниц (высота окна минус 60); columns: 1
  ...
  /* ThemePanel идёт последним — Nuklear рендерит последнее окно поверх всех */
  ui_theme_panel(ctx, S, ww, wh);
  if (S->theme_panel_anim > 0.0f)
    nk_window_set_focus(ctx, "ThemePanel");
