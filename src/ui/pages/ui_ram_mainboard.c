...
void ui_page_ram(struct nk_context *ctx, const struct AppState *S) {
                     ...
                     NK_WINDOW_NO_SCROLLBAR)) /* Top будет скроллиться */
  {
    ...
    if (nk_group_begin(ctx, "RAM", NK_WINDOW_NO_SCROLLBAR | NK_WINDOW_TITLE)) {
      ...
      nk_label(ctx, "Module manufacturer:", NK_TEXT_RIGHT);
      nk_label_val(ctx,
                   S->snap.data.ram.manufacturer[0]
                       ? S->snap.data.ram.manufacturer
                       : "-",
                   NK_TEXT_LEFT, vc);
      nk_label(ctx, "Chip manufacturer:", NK_TEXT_RIGHT);
      nk_label_val(ctx,
                   S->snap.data.ram.chip_manufacturer[0]
                       ? S->snap.data.ram.chip_manufacturer
                       : "no data",
                   NK_TEXT_LEFT, vc);

      nk_label(ctx, "Modules:", NK_TEXT_RIGHT);
      snprintf(buf, sizeof(buf), "%d", S->snap.data.ram.module_count);
      nk_label_val(ctx, buf, NK_TEXT_CENTERED, vc);
      nk_label(ctx, "Channel:", NK_TEXT_RIGHT);
      {
        const char *ch = S->snap.data.ram.channel == 1   ? "single"
                         : S->snap.data.ram.channel == 2 ? "dual"
                         : S->snap.data.ram.channel == 3 ? "triple"
                                                         : "no data";
        nk_label_val(ctx, ch, NK_TEXT_LEFT, vc);
      }
    ...
    if (nk_group_begin(ctx, "MotherBoard",
      ...
      nk_label_val(ctx, S->snap.data.board.board_vendor, NK_TEXT_CENTERED, vc);

      nk_label(ctx, "Product:", NK_TEXT_RIGHT);
      nk_label_val(ctx, S->snap.data.board.board_product, NK_TEXT_CENTERED, vc);

      nk_label(ctx, "SMBIOS UUID:", NK_TEXT_RIGHT);
      nk_label_val(ctx, S->snap.data.board.smbios_uuid, NK_TEXT_CENTERED, vc);

      nk_layout_row_dynamic(ctx, 26, 4);

      nk_label(ctx, "Bios vendor:", NK_TEXT_RIGHT);
      nk_label_val(ctx, S->snap.data.board.bios_vendor, NK_TEXT_CENTERED, vc);

      nk_label(ctx, "Bios version:", NK_TEXT_RIGHT);
      nk_label_val(ctx, S->snap.data.board.bios_version, NK_TEXT_CENTERED, vc);
  ...
  if (nk_group_begin(ctx, "RAM usage", NK_WINDOW_BORDER | NK_WINDOW_TITLE)) {
    ...
    if (S->snap.data.ram.total_mb > 0) {
      double pct = (double)S->snap.ram_rt.used_mb * 100.0 /
    ...
    nk_size total =
        (nk_size)(S->snap.data.ram.total_mb ? S->snap.data.ram.total_mb : 1);
    if (used > total)
      used = total;
